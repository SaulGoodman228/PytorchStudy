...
path_to_json = Path('..') / 'Datasets' / 'dataset_reg'

transforms = tfs.Compose([tfs.ToImage(), tfs.ToDtype(torch.float32, scale=True)])
d_train = SunDataset(path_to_json, transform=transforms)
...
d_test = SunDataset(path_to_json, train=False, transform=transforms)
